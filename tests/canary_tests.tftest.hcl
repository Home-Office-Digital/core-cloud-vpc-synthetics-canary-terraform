...
run "s3_encryption_and_lifecycle" {
  ...
  assert {
    condition     = aws_s3_bucket_server_side_encryption_configuration.canary_bucket_encryption.rule[0].apply_server_side_encryption_by_default[0].sse_algorithm == "aws:kms"
  ...
  assert {
    condition     = aws_s3_bucket_lifecycle_configuration.canary_bucket.rule[0].expiration[0].days == 30
    error_message = "Artifacts should expire after 30 days."
  }

  assert {
    condition     = aws_s3_bucket_lifecycle_configuration.canary_bucket.rule[0].noncurrent_version_expiration[0].noncurrent_days == 7
    error_message = "Non-current object versions should expire after 7 days."
  }

  assert {
    condition     = aws_s3_bucket_lifecycle_configuration.canary_bucket.rule[0].abort_incomplete_multipart_upload[0].days_after_initiation == 7
...
run "iam_and_canary_config" {
  ...
  assert {
    condition     = strcontains(aws_iam_role.canary_role.assume_role_policy, "lambda.amazonaws.com") && strcontains(aws_iam_role.canary_role.assume_role_policy, "synthetics.amazonaws.com")
